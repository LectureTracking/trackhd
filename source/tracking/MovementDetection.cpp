...
//
// Created by Maximilian Hahn on 2016/09/05.
//
...
void
MovementDetection::adjustLecturer(vector<Rect> *lecturer, vector<vector<Rect>> *memoryRects, vector<Rect> *newLect) {
    ...
    for (int i = 0; i < lecturer->size(); i++) { //for each main ghost
        ...
        if (intersected.size() == 1) {
            ...
            int newTlX = (int) (shiftR * tempGhost.tl().x) +
                         (int) (shiftRInv * memoryRects->at(i).at(intersected.at(0)).tl().x);
            int newBrX = (int) (shiftR * tempGhost.br().x) +
                         (int) (shiftRInv * memoryRects->at(i).at(intersected.at(0)).br().x);
...
void MovementDetection::findLecturer(vector<vector<Ghost>> *memoryGhosts, vector<Rect> *lecturer) {
    ...
    for (int i = 0; i < memoryGhosts->size(); i++) {
        ...
        if (memoryGhosts->at(i).size() == 0) {
            ...
            double largestVal = positionWeighting(memoryGhosts->at(i).at(0).getGhost(),
                                                  memoryGhosts->at(i).at(0).getOnScreenTime());

            int largestIdx = 0;
            for (int k = 0; k < memoryGhosts->at(i).size(); k++) {
                if (memoryGhosts->at(i).at(k).getOnScreenTime() > largestVal) {
                    largestVal = positionWeighting(memoryGhosts->at(i).at(k).getGhost(),
                                                   memoryGhosts->at(i).at(k).getOnScreenTime());
                    largestIdx = k;
                }
            }
